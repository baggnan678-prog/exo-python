...
def matrix_plot():
    try:
        ...
        if plot_type == 'heatmap':
            ...
            ax.scatter(real_parts, imag_parts, c='#ffd700', s=120, zorder=5,
            ...
            ax.plot(np.cos(theta), np.sin(theta), color='#1e1e3a', linewidth=1.5,
                    linestyle='--', alpha=0.7)
    ...
    try:
        ...
        if convert_type == 'number':
            if from_base == 'decimal':
                n_int = int(float(value)) if '.' in value else int(value)
                n_float = float(value)
            elif from_base == 'binary':
                n_int = int(value.replace(' ', ''), 2); n_float = n_int
            elif from_base == 'octal':
                n_int = int(value.replace('0o', ''), 8); n_float = n_int
            elif from_base == 'hexadecimal':
                n_int = int(value.replace('0x','').replace('0X',''), 16); n_float = n_int
            elif from_base == 'base32':
                n_int = int(value, 32); n_float = n_int
            elif from_base == 'base36':
                n_int = int(value, 36); n_float = n_int
            else:
                n_int = int(value); n_float = n_int
            ...
            try:
                ...
                result['ieee754_32'] = 'N/A'; result['ieee754_64'] = 'N/A'

        elif convert_type == 'text':
            text = value
            ascii_codes, binary_codes, hex_codes, octal_codes = [], [], [], []
            for ch in text[:200]:
                code = ord(ch)
            ...
            try: result['texte_utf8'] = bytes_val.decode('utf-8')
            except: result['texte_utf8'] = 'Non décodable en UTF-8'
            ...
            bits = value.replace(' ','')
            if len(bits)%8!=0: bits=bits.zfill(len(bits)+(8-len(bits)%8))
...
def statistics():
    try:
        ...
        if op == 'descriptive':
            ...
            return jsonify({
                'n': int(desc.nobs), 'mean': round(float(desc.mean),8),
                ...
                'min': float(dataset.min()), 'max': float(dataset.max()),
                'range': float(dataset.max()-dataset.min()),
                'Q1': round(float(q1),8), 'Q2': round(float(q2),8), 'Q3': round(float(q3),8),
            ...
            fig, axes = plt.subplots(1,2,figsize=(12,5),facecolor='#0d0d1a')
            for ax in axes:
                ax.set_facecolor('#0d0d1a'); ax.tick_params(colors='#a0a0c0')
                for sp_ in ax.spines.values(): sp_.set_color('#1e1e3a')
            axes[0].hist(dataset,bins='auto',color='#00f5ff',alpha=0.75,edgecolor='#0d0d1a',linewidth=0.5)
            axes[0].set_title('Histogramme',color='#00f5ff',fontsize=13)
            axes[0].set_xlabel('Valeurs',color='#a0a0c0'); axes[0].set_ylabel('Fréquence',color='#a0a0c0')
            axes[1].boxplot(dataset,patch_artist=True,
                ...
                whiskerprops=dict(color='#a0a0c0'), capprops=dict(color='#a0a0c0'),
            ...
            stat_ks,p_ks = stats.kstest(dataset,'norm',args=(np.mean(dataset),np.std(dataset)))
            return jsonify({
            ...
            n = len(dataset); m = np.mean(dataset); se = stats.sem(dataset)
            ci = stats.t.interval(conf,df=n-1,loc=m,scale=se)
            return jsonify({'mean':round(float(m),8),'lower':round(float(ci[0]),8),
                            'upper':round(float(ci[1]),8),'confidence':conf})

        elif op == 'ttest_one':
            mu0 = float(data.get('mu',0))
            t_stat,p_val = stats.ttest_1samp(dataset,mu0)
            return jsonify({'t_statistic':round(float(t_stat),6),'p_value':round(float(p_val),6),
            ...
            return jsonify({'pearson_r':round(float(r),8),'pearson_p':round(float(p),8),
                            'spearman_rho':round(float(rho),8),'spearman_p':round(float(p2),8)})
            ...
            plt.savefig(buf,format='png',dpi=130,bbox_inches='tight',facecolor='#0d0d1a')
            plt.close(); buf.seek(0)
            return jsonify({'slope':round(float(slope),8),'intercept':round(float(intercept),8),
                            'r_squared':round(float(r_value**2),8),'p_value':round(float(p_value),8),
...
def probability():
    try:
        ...
        if action == 'pdf':
            ...
            result = {'mean':round(float(m),8),'variance':round(float(v),8),
                      'std':round(float(np.sqrt(v)),8),'skewness':round(float(s),8),
                      'kurtosis':round(float(k),8)}
        elif action == 'plot':
            fig,axes = plt.subplots(1,2,figsize=(12,5),facecolor='#0d0d1a')
            is_discrete = not hasattr(d,'pdf')
            mean,var = d.stats(); std = float(np.sqrt(var)); mean = float(mean)
            for ax in axes:
                ax.set_facecolor('#0d0d1a'); ax.tick_params(colors='#a0a0c0')
                for sp_ in ax.spines.values(): sp_.set_color('#1e1e3a')
                ax.grid(True,color='#1e1e3a',alpha=0.5)
            if is_discrete:
                ...
                axes[0].bar(x_range,d.pmf(x_range),color='#00f5ff',alpha=0.8,edgecolor='#0d0d1a')
                ...
                axes[1].set_title('CDF',color='#00f5ff')
            for ax in axes:
                ax.set_xlabel('x',color='#a0a0c0'); ax.set_ylabel('Probabilité',color='#a0a0c0')
            plt.suptitle(f'Distribution {dist.capitalize()}',color='#00f5ff',fontsize=14,y=1.01)
            plt.tight_layout()
            buf = io.BytesIO()
            plt.savefig(buf,format='png',dpi=130,bbox_inches='tight',facecolor='#0d0d1a')
            plt.close(); buf.seek(0)
